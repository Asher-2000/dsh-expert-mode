...
### dsh-expert-mode-pro-ui 插件 index.js 导出错误

**现象**：DSH 启动崩溃，日志报 `SyntaxError: The requested module './client.js' does not provide an export named 'default'`

**原因**：`lib/index.js` 错误地用了 `export { default as ClientPlugin } from './client.js'`，但 `client.js` 是纯浏览器端代码，没有 `default` 导出。DSH 插件的 node 端入口（`index.js`）应该导出 `apply` 和 `name`，UI 逻辑由 `client.js`（浏览器 bundle）通过 `dsh.client` 声明加载。

**修复**：
```js
// ❌ 错误
export { default as ClientPlugin } from './client.js';

// ✅ 正确 — node 端是 no-op
const name = 'dsh-expert-mode-pro-ui';
function apply() {}
export { apply, name };
```

**教训**：DSH Cordis 插件的 `index.js`（node 端）只负责注册插件名，不需要 re-export 浏览器端代码。`dsh.client` 声明的 `client.js` 由 web shell 的 client-modules loader 自动加载。
